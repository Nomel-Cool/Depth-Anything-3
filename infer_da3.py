    ...
    with torch.no_grad():
        for i in tqdm(range(len(test_images))):
            ...
            with autocast_ctx:
                ...
                # # === 保存预处理后的输入图片用于调试 ===
                # debug_input_dir = os.path.join(args.output_dir, "input_preprocessed")
                # os.makedirs(debug_input_dir, exist_ok=True)
                
                # # 1. 从 GPU 拿到 CPU，去掉 batch 维度
                # input_to_save = test_image.detach().cpu().squeeze(0) # 变为 [3, H, W]
                # # 2. 反归一化:从 [-1, 1] 还原到 [0, 255]
                # input_to_save = ((input_to_save + 1.0) * 127.5).clamp(0, 255)
                # # 3. 维度转换: [3, H, W] -> [H, W, 3]
                # input_to_save = input_to_save.permute(1, 2, 0).numpy().astype(np.uint8)
                # # 4. 使用 PIL 保存
                # input_pil = Image.fromarray(input_to_save)
                # input_pil.save(os.path.join(debug_input_dir, os.path.basename(test_images[i])))
                # # =====================================
                ...
                if args.task_name == 'depth':
                    ...
                    p_low, p_high = np.percentile(output_npy, [0, 95])
                    depth_norm = np.clip((output_npy - p_low) / (p_high - p_low + 1e-6), 0, 1)
